import { IconProps } from "@/types/icons";
import React from "react";

const NotificationIcon = ({ isActive, width, height }: IconProps) => {
  const containerStyle: Partial<IconProps> = {};

  if (width) containerStyle.width = width;
  if (height) containerStyle.height = height;

  if (isActive)
    return (
      <div style={containerStyle}>
        <svg
          width="100%"
          height="100%"
          viewBox="0 0 18 20"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M16.7695 9.64534C16.039 8.79229 15.7071 8.05305 15.7071 6.79716V6.37013C15.7071 4.73354 15.3304 3.67907 14.5115 2.62459C13.2493 0.986993 11.1244 0 9.04423 0H8.95577C6.91935 0 4.86106 0.941671 3.577 2.5128C2.71333 3.58842 2.29293 4.68822 2.29293 6.37013V6.79716C2.29293 8.05305 1.98284 8.79229 1.23049 9.64534C0.676907 10.2738 0.5 11.0815 0.5 11.9557C0.5 12.8309 0.787226 13.6598 1.36367 14.3336C2.11602 15.1413 3.17846 15.6569 4.26375 15.7466C5.83505 15.9258 7.40634 15.9933 9.0005 15.9933C10.5937 15.9933 12.165 15.8805 13.7372 15.7466C14.8215 15.6569 15.884 15.1413 16.6363 14.3336C17.2118 13.6598 17.5 12.8309 17.5 11.9557C17.5 11.0815 17.3231 10.2738 16.7695 9.64534Z"
            fill="#554AF0"
          />
          <path
            opacity="0.4"
            d="M11.0087 17.2284C10.5088 17.1216 7.4626 17.1216 6.96269 17.2284C6.53533 17.3271 6.07318 17.5567 6.07318 18.0602C6.09803 18.5406 6.37929 18.9647 6.76888 19.2336L6.76789 19.2346C7.27178 19.6273 7.86313 19.8771 8.4823 19.9668C8.81226 20.0121 9.14819 20.0101 9.49008 19.9668C10.1083 19.8771 10.6996 19.6273 11.2035 19.2346L11.2025 19.2336C11.5921 18.9647 11.8734 18.5406 11.8982 18.0602C11.8982 17.5567 11.4361 17.3271 11.0087 17.2284Z"
            fill="#554AF0"
          />
        </svg>
      </div>
    );

  return (
    <div style={containerStyle}>
      <svg
        width="100%"
        height="100%"
        viewBox="0 0 20 22"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          fill-rule="evenodd"
          clip-rule="evenodd"
          d="M10 16.8476C15.6392 16.8476 18.2481 16.1242 18.5 13.2205C18.5 10.3188 16.6812 10.5054 16.6812 6.94511C16.6812 4.16414 14.0452 1 10 1C5.95477 1 3.31885 4.16414 3.31885 6.94511C3.31885 10.5054 1.5 10.3188 1.5 13.2205C1.75295 16.1352 4.36177 16.8476 10 16.8476Z"
          stroke="#686777"
          stroke-width="1.5"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
        <path
          d="M12.3888 19.8572C11.0247 21.372 8.89665 21.3899 7.51947 19.8572"
          stroke="#686777"
          stroke-width="1.5"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
      </svg>
    </div>
  );
};

export default NotificationIcon;
